...
function buildRoutes(app, specs, enforcer) {
  _.each(specs, (service, name) => {
    if (service.whitelisted) {
      app.get(`/${name}*`, handler.bind(handler, enforcer))
      app.post(`/${name}*`, handler.bind(handler, enforcer))
      app.put(`/${name}*`, handler.bind(handler, enforcer))
      app.delete(`/${name}*`, handler.bind(handler, enforcer))
    } else {
